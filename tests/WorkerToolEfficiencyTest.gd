...
func _test_farmer_role_and_sickle_income(build_manager: Node2D, npc_manager: Node2D, npcs: Node2D, buildings: Node2D) -> void:
	...
	build_manager._update_farm_income(39.0)
	_assert_equal(build_manager.gold, 0, "wood sickle does not pay before forty seconds")
	build_manager._update_farm_income(1.0)
	_assert_equal(build_manager.gold, 1, "wood sickle improves farm income speed by fifty percent")
...
func _test_tree_chop_duration_and_axe(build_manager: Node2D, npc_manager: Node2D, npcs: Node2D, buildings: Node2D) -> void:
	...
	npc_manager._finish_arriving_tree_choppers()
	build_manager.gold = 0
	npc_manager._advance_tree_choppers(39.0)
	_assert_equal(build_manager.gold, 0, "wood axe does not finish before forty seconds")
	npc_manager._advance_tree_choppers(1.0)
	_assert_equal(build_manager.gold, 1, "wood axe improves tree chopping speed by fifty percent")

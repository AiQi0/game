...
func _test_catalog(catalog) -> void:
	...
	_assert_equal(buildings.size(), 10, "catalog has base, support, and terrain build targets")
	_assert_equal(buildings[0].id, "blacksmith", "slot 1 is blacksmith")
	_assert_equal(buildings[0].get("cost"), 10, "blacksmith costs 10 gold")
	_assert_equal(buildings[1].id, "wall", "slot 2 is wall")
	_assert_equal(buildings[1].get("cost"), 5, "wall costs 5 gold")
	_assert_equal(buildings[2].id, "tavern", "slot 3 is tavern")
	_assert_equal(buildings[2].get("cost"), 20, "tavern costs 20 gold")
	_assert_equal(buildings[3].id, "post_station", "catalog includes post station")
	_assert_equal(buildings[3].keycode, KEY_4, "post station uses top-row 4")
	_assert_equal(buildings[4].id, "barracks", "catalog includes barracks")
	_assert_equal(buildings[4].keycode, KEY_5, "barracks uses top-row 5")
	_assert_equal(buildings[5].id, "river_port", "catalog includes river port")
	_assert_equal(buildings[6].id, "beacon_tower", "catalog includes northern beacon tower")
	_assert_equal(buildings[8].id, "iron_mine", "catalog includes mountain iron mine")
	_assert_equal(buildings[0].keycode, KEY_1, "blacksmith uses top-row 1")
	_assert_equal(buildings[2].keycode, KEY_3, "tavern uses top-row 3")
...
func _test_number_row_selection(rules) -> void:
	...
	_assert_equal(rules.selected_index_from_key(KEY_3), 2, "top-row 3 selects tavern")
	_assert_equal(rules.selected_index_from_key(KEY_4), 3, "top-row 4 selects post station")
	_assert_equal(rules.selected_index_from_key(KEY_5), 4, "top-row 5 selects barracks")
	_assert_equal(rules.selected_index_from_key(KEY_6), 5, "top-row 6 selects first terrain slot")
	_assert_equal(rules.selected_index_from_key(KEY_0), 9, "top-row 0 selects tenth slot")
	...
	_assert_equal(rules.selected_index_from_key(KEY_KP_6), -1, "keypad 6 does not select a slot")

...
const STARTING_HOMELESS_RANDOM_SEED := RANDOM_SEED + 101
const NPC_TREE_CHOP_SECONDS := 60.0
const TOOL_EFFICIENCY_MULTIPLIER := 2.0
...
func _equip_workers_from_tools() -> void:
	...
	for child in npc_container.get_children():
		...
		if (pickup.get("tool_id", "") == "sword" or pickup.get("tool_id", "") == "bow") and build_manager.has_method("release_work_site_for_worker"):
...
func _finish_arriving_workers() -> void:
	...
	for child in npc_container.get_children():
		...
		if build_manager.occupy_work_site(workplace_id, child.name):
			...
			elif workplace_id.begins_with("lumberyard") and child.has_method("become_lumberjack"):
...
func _advance_tree_choppers(delta: float) -> void:
	...
	for child in npc_container.get_children():
		...
		var duration := NPC_TREE_CHOP_SECONDS
		if child.get("carried_tool") == "axe":
			duration = NPC_TREE_CHOP_SECONDS / TOOL_EFFICIENCY_MULTIPLIER
